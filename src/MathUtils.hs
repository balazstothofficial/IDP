...
import Data.Matrix
import Data.Random
import qualified Data.Vector as DV (Vector, foldr, fromList, length, toList, zip)
...
sumRows m = map (sum . rowList) [1 .. nrows m]
  where
    rowList i' = DV.toList $ getRow i' m
...
updateRow i m v = foldl (fun i) m (zip [1 .. ncols m] (DV.toList v))
  where
    fun :: Int -> Matrix a -> (Int, a) -> Matrix a
...
updateCol um (umInd, mInd) m = condense $ swapCols $ m <|> um
  where
    condense = submatrix 1 (nrows m) 1 (ncols m)
